...
async function uniqueInviteCode(): Promise<string> {
  for (let i = 0; i < 10; i++) {
    ...
    const existing = await leaguesRepo.getLeagueByInvite(code);
...
export class LeaguesService {
  async createLeague(params: {
    ...
    await leaguesRepo.createLeague(league);
    await leaguesRepo.addMember({
      leagueId: league.id,
    ...
    return (await this.getLeague(league.id))!;
  }

  async getLeague(leagueId: string): Promise<LeagueSummary | undefined> {
    const league = await leaguesRepo.getLeague(leagueId);
    if (!league) return undefined;
    const members = await leaguesRepo.listMembers(leagueId);
    return { ...league, members, memberCount: members.length };
  }

  async listUserLeagues(userId: string): Promise<LeagueSummary[]> {
    const leagues = await leaguesRepo.listLeaguesForUser(userId);
    // Fetch members for each in parallel to amortize the network roundtrip
    // cost over the fanout.
    return Promise.all(leagues.map(async (l) => {
      const members = await leaguesRepo.listMembers(l.id);
      return { ...l, members, memberCount: members.length };
    }));
  }

  async joinByCode(params: { userId: string; inviteCode: string; teamName?: string }):
    Promise<{ ok: true; league: LeagueSummary } | { ok: false; error: string }> {
    const code = params.inviteCode.trim().toUpperCase();
    const league = await leaguesRepo.getLeagueByInvite(code);
    if (!league) return { ok: false, error: "Invite code not found" };
    const existing = await leaguesRepo.getMember(league.id, params.userId);
    if (existing) return { ok: true, league: (await this.getLeague(league.id))! };
    const members = await leaguesRepo.listMembers(league.id);
    ...
    return { ok: true, league: (await this.getLeague(league.id))! };
  }
   ...
   */
  async generateSchedule(leagueId: string, weeks = DEFAULT_SEASON_WEEKS): Promise<number> {
    const league = await leaguesRepo.getLeague(leagueId);
    if (!league) return 0;
    const existing = await leaguesRepo.listMatchups(leagueId);
  ...
  async getStandings(leagueId: string): Promise<LeagueMemberRecord[]> {
    const members = await leaguesRepo.listMembers(leagueId);
  ...
  async getWeekMatchups(leagueId: string, week?: number): Promise<MatchupRecord[]> {
    const league = await leaguesRepo.getLeague(leagueId);
    if (!league) return [];
   ...
   */
  async scoreMember(leagueId: string, userId: string): Promise<number> {
    const league = await leaguesRepo.getLeague(leagueId);
    if (!league) return 0;
    const roster = await rosterRepo.getBySport(userId, league.sport);
   ...
   */
  async settleWeek(leagueId: string, week: number): Promise<{ settled: number }> {
    const league = await leaguesRepo.getLeague(leagueId);
    if (!league) return { settled: 0 };
    const weekMatchups = await leaguesRepo.listMatchups(leagueId, week);
    let settled = 0;
    for (const m of weekMatchups) {
      if (m.status === "final") continue;
      if (m.homeUserId === m.awayUserId) continue;
      const homeScore = Math.round((await this.scoreMember(leagueId, m.homeUserId)) * 100) / 100;
      const awayScore = Math.round((await this.scoreMember(leagueId, m.awayUserId)) * 100) / 100;
      await leaguesRepo.updateMatchup(m.id, {
        status: "final",
        homeScore,
        awayScore,
        settledAt: new Date().toISOString(),
      });
      const home = await leaguesRepo.getMember(leagueId, m.homeUserId);
      const away = await leaguesRepo.getMember(leagueId, m.awayUserId);
      if (home && away) {
        ...
        await leaguesRepo.updateMember(leagueId, m.homeUserId, homePatch);
        await leaguesRepo.updateMember(leagueId, m.awayUserId, awayPatch);
      }
      settled++;
    }
    if (settled > 0) {
      await leaguesRepo.updateLeague(leagueId, { currentWeek: week + 1 });
    }
    return { settled };
  }
}
